...
def test_visualization_namespace_derivation() -> None:
    ...
    assert ns.mongo_collection("analysis_results") == "ai26__analysis_results"
...
def test_settings_default_mongodb_collection_is_project_scoped() -> None:
    ...
    assert settings.resolved_mongodb_collection == "ep24__analysis_results"
...
def test_named_projects_do_not_collide() -> None:
    ...
    assert len({item.mongo_collection("analysis_results") for item in namespaces}) == len(names)
