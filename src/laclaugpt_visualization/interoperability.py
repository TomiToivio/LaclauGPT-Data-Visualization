...
def dats_review_view(project: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for ann in project.get("annotations", []):
        ...
        review_status = str(
            ...
            or ("ACCEPTED" if producer_type == "human" else "PROVISIONAL")
        )
...
def dna_graph_view(
    statements: Iterable[Mapping[str, Any]], projection: Mapping[str, Any]
) -> GraphViewModel:
        ...
        node_writer.writerow(
            {
                ...
                "review_status": node.review_status or "",
        ...
        edge_writer.writerow(
            {
                ...
                "review_status": edge.review_status or "",
                "uncertainty": edge.uncertainty or "",
            }
        )
    return node_buffer.getvalue(), edge_buffer.getvalue()


def _semantic_metadata(view: GraphViewModel) -> dict[str, Any]:
    return {
...
def graphml_dumps(view: GraphViewModel) -> str:
    ...
    graph = SubElement(
        ...
        "graph",
        {
            ...
            "edgedefault": "directed" if any(edge.directed for edge in view.edges) else "undirected",
        },
    )
    SubElement(graph, "data", {"key": "semantic_metadata"}).text = json.dumps(
        _semantic_metadata(view), ensure_ascii=False, sort_keys=True
    )
    for node in view.nodes:
...
def gexf_dumps(view: GraphViewModel) -> str:
    ...
    graph = SubElement(
        ...
        "graph",
        {
            ...
            if any(edge.directed for edge in view.edges)
            else "undirected",
        },
    )
    SubElement(graph, "meta").text = json.dumps(
        _semantic_metadata(view), ensure_ascii=False, sort_keys=True
    )
    nodes = SubElement(graph, "nodes")
    edges = SubElement(graph, "edges")
    for node in view.nodes:
        SubElement(nodes, "node", {"id": node.id, "label": node.label, "type": node.node_type})
    for edge in view.edges:
        SubElement(
            edges,
            "edge",
...
def renderer_payload(view: GraphViewModel, renderer: str = "cytoscape") -> dict[str, Any]:
    ...
    metadata = _semantic_metadata(view)
    ...
    if renderer in {"sigma", "graphology"}:
        return {
            ...
            "nodes": [
                {
                    "key": node.id,
                    "attributes": {"label": node.label, "node_type": node.node_type},
                }
                for node in view.nodes
            ],
            "edges": [
                {
                    ...
                    "source": edge.source,
                    "target": edge.target,
                    "attributes": {"type": edge.edge_type, "weight": edge.weight},
                }
                for edge in view.edges
            ],
            "metadata": metadata,
...
def vega_lite_spec(
    ...
    *,
    ...
    return {
        ...
        "description": (
            "Renderer-neutral LaclauGPT chart. Visual prominence is descriptive, "
            "not a theoretical conclusion."
        ),
        "title": title,
