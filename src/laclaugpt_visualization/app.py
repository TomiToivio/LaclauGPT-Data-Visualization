...
from .storage import artifact_references, download_s3_object, load_mongodb, resolve_artifact_reference
...
def _render_artifacts(row: Any) -> None:
    """Render safe canonical artifact references and fetch only after explicit user action."""
    settings = get_settings()
    ...
    for index, value in enumerate(references):
        ...
        if not artifact.downloadable or not artifact.key:
            st.caption(artifact.reason or "Reference is not downloadable through configured object storage.")
            continue
        if st.button("Resolve from object storage", key=f"artifact-{index}-{artifact.key}"):
            ...
            try:
                ...
                # Keep credentials/endpoints out of the UI: only the exception type is exposed.
