...
def _resource_uri(node: Mapping[str, object]) -> str:
...
with query_tab:
    ...
    sparql = st.text_area(
        "SPARQL", value=default_query, height=220, disabled=not advanced and bool(selected)
    )
    c1, c2 = st.columns(2)
...
with provenance_tab:
    ...
    if policy.graphrag_enabled:
        ...
        st.caption(
            "Shows selected graph paths/nodes and source evidence; it does not replace other retrieval providers."
        )
        ...
        st.caption(
            "RDF GraphRAG is disabled by project policy; ordinary RDF browsing/export remains available."
        )
